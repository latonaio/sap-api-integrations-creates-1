...
import (
	...
	"sap-api-integrations-xxxxxx-xxxxxx-creates/SAP_API_Caller/responses"
...
func (c *SAPAPICaller) HeaderItem(headerItem *requests.HeaderItem) {
	err := c.callXXXXXSrvAPIRequirementHeaderItem("A_XXXXX", headerItem)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callXXXXXSrvAPIRequirementHeaderItem(api string, headerItem *requests.HeaderItem) error {
	body, err := json.Marshal(headerItem)
	if err != nil {
		return xerrors.Errorf("API request error: %w", err)
	}
	url := strings.Join([]string{c.baseURL, "API_XXXXX_SRV", api}, "/")
	...
	headerItem.XXXXX = resBody.D.XXXXX
	return nil
}
